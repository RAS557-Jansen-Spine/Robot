...
Sarrus linkage, we develop a modified planar parallel linkage that connects two
rigid 3D-printed chassis plates and is intended to provide vertical support
...
mechanism and a laminate prototype integrated into a quadruped chassis. A
planar kinematic model, implemented in Python, approximates the
six-link Sarrus-based spine as an equivalent three-joint serial chain in the
sagittal plane. Using this model, we compute representative joint torques and
...
spine for a small quadruped robot driven by Jansen mechanisms at each leg. The
design is inspired by the classical Sarrus linkage but modified into a
planar parallel structure that connects two rigid 3D-printed chassis plates.
...
Several prior works inform the specific design and fabrication choices in this
project.  
Hoover and Fearing introduced the Smart Composite Microstructures process for
laminate robots and demonstrated complex linkage mechanisms fabricated from
FR4 and Kapton layers~\cite{hoover_fearing}. Eckert \emph{et al.}
experimentally explored the coupling between leg design and spine stiffness in
small quadrupeds~\cite{eckert}. Bing \emph{et al.} showed that lateral spinal
flexion can improve turning behavior and performance in small legged
robots~\cite{bing}. D\"urr \emph{et al.} provided a comprehensive survey of
legged locomotion and inter-limb coordination for multi-legged animals, which
helps motivate our long-term goal of integrating the articulated spine into a
biologically grounded gait framework~\cite{duerr}. The textbook
\emph{Foldable Robotics} by Aukes~\cite{aukes_textbook} provides the
lamination and hinge design rules used in our five-layer laminate spine.
...
mechanism using slit-and-fold joints. Figure~\ref{fig:images/fig_prototype} shows the
...
\begin{figure}[H]
  ...
  \label{fig:images/fig_prototype}
\end{figure}
...
The physical spine module is a six-link Sarrus-based mechanism between two
rigid platforms (front and rear chassis). For clarity, we draw a planar
...
\begin{figure}[H]
  ...
  $\hat{n}_x$ axis. The full spatial Sarrus linkage has six moving links; for
  planar kinematic analysis we later approximate one branch as a 3R serial
  chain.}
...
To analyze the articulated spine, we implemented a planar kinematic model in
Python using SymPy and Matplotlib. For tractability, the spatial six-link
Sarrus spine is approximated as an equivalent planar 3R serial chain that
connects the rear chassis attachment to the effective front chassis
attachment. Each branch of the Sarrus linkage follows the same kinematics, so
the serial representation captures the dominant bending motion and the mapping
between endpoint forces and joint torques.

\subsection{Variables, Parameters, and Frames}

The model is defined in an inertial frame $\mathcal{N}$ with orthonormal basis
$\{\hat{n}_x,\hat{n}_y,\hat{n}_z\}$, where $\hat{n}_x$ is horizontal and
$\hat{n}_y$ is vertical. The base of the spine at the rear chassis is fixed at
\begin{equation}
    P_0 =
    \begin{bmatrix}
        0 \\ 0
    \end{bmatrix}.
\end{equation}
The equivalent spine has three rigid links of lengths $l_1,l_2,l_3$ with
revolute joints at $P_0$, $P_1$, and $P_2$. The generalized coordinates are
\begin{equation}
    \mathbf{q} =
    \begin{bmatrix}
...
\end{equation}
where each $\theta_i$ is a relative joint angle. The absolute orientations are
\begin{equation}
    \alpha_1 = \theta_1,\quad
    \alpha_2 = \theta_1 + \theta_2,\quad
    \alpha_3 = \theta_1 + \theta_2 + \theta_3.
\end{equation}
Link lengths are chosen based on the prototype:
$l_1 = l_2 = \SI{0.06}{m}$ and $l_3 = \SI{0.05}{m}$.

\subsection{Forward Kinematics and Configuration Plots}

Using a vector-based description, the joint positions are
\begin{align}
    P_1 &= P_0 +
    l_1
    \begin{bmatrix}
    ...
    \end{bmatrix},
\end{align}
where $P_3$ is the effective front chassis location. The Python script
evaluates these expressions symbolically, then numerically for several key
configurations:
\begin{itemize}
    \item Neutral posture:
    $\mathbf{q}_{\text{neutral}} =
    [0, 0, 0]^{\mathsf{T}}$.
    \item Left-bending posture:
    $\mathbf{q}_{\text{left}} =
    [10^\circ,-5^\circ,0]^{\mathsf{T}}$.
    \item Right-bending posture:
    $\mathbf{q}_{\text{right}} =
    [-10^\circ,5^\circ,0]^{\mathsf{T}}$.
\end{itemize}
Plots of these configurations show that the model produces a total spine pitch
of approximately $\pm 10^\circ$, consistent with the design target.

\subsection{Jacobian and Force Mapping}
...
\begin{equation}
    ...
    \begin{bmatrix}
        ...
        \frac{\partial x}{\partial\theta_3} \\
        \frac{\partial y}{\partial\theta_1} &
        \frac{\partial y}{\partial\theta_2} &
        \frac{\partial y}{\partial\theta_3}
    \end{bmatrix}.
\end{equation}
For static analysis, the relationship between endpoint forces and joint torques
is
\begin{equation}
    \boldsymbol{\tau} = J^{\mathsf{T}}\mathbf{F}_{\text{ee}},
\end{equation}
where $\mathbf{F}_{\text{ee}}$ is the force at the endpoint. Using the
specifications in Sec.~\ref{sec:specs}, we apply a downward load
$\mathbf{F}_{\text{ee}} = [0,\,-4]^{\mathsf{T}}$\,N and compute the resulting
joint torques at each configuration. The magnitudes of these torques are
within the range that can be provided by small hobby servomotors, confirming
that the laminated spine is structurally reasonable.

\subsection{Jacobian and Velocity Mapping}

The Jacobian also relates endpoint velocities to joint velocities:
\begin{equation}
    \dot{\mathbf{r}} = J(\mathbf{q}) \dot{\mathbf{q}}.
\end{equation}
Given a desired endpoint velocity, e.g.
$\dot{\mathbf{r}} = [0, 0.05]^{\mathsf{T}}$\,\si{m/s} at the neutral
configuration, the script uses the Jacobian pseudoinverse to compute a
minimum-norm joint velocity vector $\dot{\mathbf{q}}$. This provides a
first-order estimate of the required joint speeds to achieve a given vertical
motion of the front chassis.

\subsection{Python Implementation}

The complete Python script is included in Appendix~\ref{app:code} and is based
on the vector-kinematics examples in the course notebook. It defines all
variables and constants, declares frames implicitly through link orientations,
computes joint positions and Jacobians symbolically using SymPy, and
visualizes the mechanism in key configurations using Matplotlib.

\section{Discussion and Future Work}
\label{sec:discussion}

The articulated spine developed in this project demonstrates that a Sarrus-based
laminate mechanism can provide directional stiffness and passive compliance for
a cat-scale quadruped robot. The paper prototype helped refine the physical
design and link layout, while the laminate prototype shows that the five-layer
stack can achieve the necessary stiffness-to-compliance ratio using standard
SCM materials.

The simplified planar 3R kinematic model provides a useful approximation for
understanding how body-level forces map into joint torques and how desired
spine motions relate to joint velocities. However, the full spatial Sarrus
mechanism contains additional geometric and stiffness details that are not
captured in the planar model. Future work will include:
\begin{itemize}
    \item Extending the model to full 3D, including lateral bending and
    torsion.
    \item Integrating Jansen mechanisms on each chassis and studying the
    coupled leg--spine dynamics.
    \item Calibrating the effective spine stiffness by measuring torque versus
    angle for the laminate flexures and comparing to the target stiffness
    estimated in Sec.~\ref{sec:goal_metrics}.
    \item Implementing closed-loop control strategies that exploit the
    passive compliance for perturbation rejection.
\end{itemize}
...
maintaining vertical support. A Python-based kinematic model, derived from an
equivalent planar 3R chain, provides estimates of joint torques under expected
